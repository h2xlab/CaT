...
from nav_planner import PIDController, RoutePlanner, interpolate_trajectory, LavRoutePlanner
...
def world_to_pixel(x,y,ox,oy,ori_ox, ori_oy, offset=(-40,160)):
    PIXELS_PER_METER = 4
...
WorldOffset = {
    'Town01': (-162.05990600585938, -162.0499608516693),
    'Town02': (-167.4597291946411, -54.6092529296875),
    'Town03': (-309.0638427734375, -369.27125549316406),
    'Town04': (-675.26904296875, -556.1461181640625),
    'Town05': (-436.0445251464844, -367.8750915527344),
    'Town06': (-530.7425537109375, -312.3881530761719),
    'Town07': (-364.62701416015625, -408.9809265136719),
    'Town10HD': (-274.59522247314453, -228.72904205322266),
    }
...
class AutoPilot(autonomous_agent_local.AutonomousAgent):
    def setup(self, path_to_conf_file, route_index=None):
        ...
        self.save_freq = 5 # By default, save once every 5 frames (0.25 seconds)
        ...
        self.hazard_source = {"stop_sign": [], "traffic_light": [], "walker": [], "vehicle":[]}
    ...
    def _init(self, hd_map):
        ...
        print('Map: ', CarlaDataProvider._map.name)
        ...
        # lav planner
        self.lavplanner = LavRoutePlanner(self._global_plan)
    ...
    def autotick(self, input_data):
        ...
        # get nxps
        wx, wy = self.lavplanner.run_step(gps)
        # Transform to ego-coordinates
        wx, wy = _rotate(wx, wy, -compass+np.pi/2)
    ...
    def run_step(self, input_data, timestamp):
        ...
        tick_data_tmp = self.autotick(input_data)
    ...
    def _get_control(self, input_data, steer=None, throttle=None,
        ...
        pos = self.auto_get_position(input_data['gps'][1][:2])
        ...
        if steer is None:
            ...
            if (math.isnan(theta) == True): # simulation bug
                theta = 0.0
        ...
        if self.render_bev==False:
            tick_data = self.autotick(input_data)
        else:
            tick_data = self.autotick(input_data, self.future_states)
    ...
    def _get_brake(self, vehicle_hazard=None, light_hazard=None, walker_hazard=None, stop_sign_hazard=None):
        ...
        if light_hazard is None:
            light_hazard = False
            self.hazard_source['traffic_light'] = []
            ...
            for light in _traffic_lights:
                ...
                if(self.check_obb_intersection(light_detector_bb, bounding_box) == True):
                    if ((light.state == carla.libcarla.TrafficLightState.Red)
                        ...
                        light_hazard = True
                        self.hazard_source['traffic_light'].append(light)
                    ...
                    if (self.check_obb_intersection(vehicle_stop_sign_detector_bb, bounding_box_stop_sign) == True):
                        if(not (stop_sign.id in self.cleared_stop_signs)):
                            if((speed * 3.6) > 0.0): #Conversion from m/s to km/h
                                ...
                                self.hazard_source['stop_sign'].append(stop_sign)
                                color = carla.Color(255, 0, 0, 255)
                            else:
        ...
        if vehicle_hazard is None or walker_hazard is None:
            ...
            for walker in walkers:
                if (walker.get_location().distance(vehicle_location) < detection_radius):
                    ...
                    nearby_walkers.append((walker, walker_future_bbs))
                ...
                for id, traffic_participant in nearby_vehicles.items():
                    ...
                    if front_intersect:
                        ...
                        if self.junction==True or i <= number_of_future_frames_no_junction:
                            vehicle_hazard = True
                            self.hazard_source['vehicle'].append(self._world.get_actor(id))
                        self.vehicle_hazard[i] = True
                    
                for actor_handle, walker in nearby_walkers:
                    if self.render_bev==False and self.junction==False and i > number_of_future_frames_no_junction:
                            break
                    if (self.check_obb_intersection(bounding_box, walker[i]) == True):
                        ...
                        if self.junction==True or i <= number_of_future_frames_no_junction:
                            walker_hazard = True
                            self.hazard_source['walker'].append(actor_handle)
            ...
            _nearby_walkers = [w[1] for w in nearby_walkers]
            self.future_states = {'walker': _nearby_walkers, 'vehicle': nearby_vehicles}
    ...
    def auto_get_position(self, gps):
