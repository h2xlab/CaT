...
class ObsManager(ObsManagerBase):
    def __init__(self, obs_configs):
        ...
        self._frame_rate = 20    # fixed carla frame rate
    ...
    def get_observation(self, hazard_source):
        ...
        def is_within_distance_actor(actor):
            w = actor.get_transform()
            c_distance = abs(ev_loc.x - w.location.x) < self._distance_threshold \
                and abs(ev_loc.y - w.location.y) < self._distance_threshold \
                and abs(ev_loc.z - w.location.z) < 8.0
            return c_distance
        ...
        # get hazard mask
        hazard_mask = np.zeros([self._width, self._width], dtype=np.bool)
        for hazard_type, hazard_actors in hazard_source.items():
            if len(hazard_actors) > 0:
                if self._scale_bbox:
                    if hazard_type == "walker":
                        hazard_mask = hazard_mask | self._get_mask_from_actor_handle(hazard_actors, hazard_type, is_within_distance_actor, M_warp, 2.0)
                    else:
                        hazard_mask = hazard_mask | self._get_mask_from_actor_handle(hazard_actors, hazard_type, is_within_distance_actor, M_warp, 1.0)
                else:
                    hazard_mask = hazard_mask | self._get_mask_from_actor_handle(hazard_actors, hazard_type, is_within_distance_actor, M_warp)
        ...
        image[hazard_mask] = COLOR_ORANGE
        ...
        c_hazard = hazard_mask * 255

        masks = np.stack((c_road, c_route, c_lane, *c_vehicle_history, *c_walker_history, *c_tl_history, *c_vehicle_future, c_hazard), axis=2)
    ...
    def _get_mask_from_actor_handle(self, actor_list, actor_type, criteria, M_warp, scale=None):
        actors = []
        for actor in actor_list:
            is_within_distance = criteria(actor)
            if is_within_distance:
                trans = actor.get_transform()
                if actor_type == "stop_sign":
                    bb_loc = carla.Location(actor.trigger_volume.location)
                    bb_ext = carla.Vector3D(actor.trigger_volume.extent)
                    bb_ext.x = max(bb_ext.x, bb_ext.y)
                    bb_ext.y = max(bb_ext.x, bb_ext.y)
                    actors.append((carla.Transform(trans.location, trans.rotation), bb_loc, bb_ext))
                    ...
                    for i in range(TrafficLightHandler.num_tl):
                        if actor.id == TrafficLightHandler.list_tl_actor[i].id:
                            actors += TrafficLightHandler.list_stopline_vtx[i]
                else:
                    pass # do nothing            
        
        # get mask
        if actor_type == "traffic_light":
            mask = self._get_mask_from_stopline_vtx(actors, M_warp)
        else:
            mask = self._get_mask_from_actor_list(actors, M_warp)
        return mask
