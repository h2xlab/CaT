...
class RefPntsNoGradGenerator(nn.Module):
    def __init__(self, size_ipm, M_r, M_m, M_l, no_cuda):
        """
    ...
    def forward(self, bs, device):
        ...
        M_inv_r = torch.zeros(bs, 3, 3)
        M_inv_m = torch.zeros(bs, 3, 3)
        M_inv_l = torch.zeros(bs, 3, 3)

        for i in range(bs):
            M_inv_r[i] = torch.from_numpy(self.M_r).type(torch.FloatTensor)
            M_inv_m[i] = torch.from_numpy(self.M_m).type(torch.FloatTensor)
            M_inv_l[i] = torch.from_numpy(self.M_l).type(torch.FloatTensor)

        M_inv_r = M_inv_r.to(device)
        M_inv_m = M_inv_m.to(device)
        M_inv_l = M_inv_l.to(device)

        self.base_grid = self.base_grid.to(device)

        with torch.no_grad():
            grid_r = torch.matmul(self.base_grid.view(self.H * self.W, 3), M_inv_r.transpose(1, 2))
            grid_r = torch.div(grid_r[:, :, 0:2], grid_r[:, :, 2:])
            _grid_r = grid_r.clone()

            grid_m = torch.matmul(self.base_grid.view(self.H * self.W, 3), M_inv_m.transpose(1, 2))
            grid_m = torch.div(grid_m[:, :, 0:2], grid_m[:, :, 2:])
            _grid_m = grid_m.clone()

            grid_l = torch.matmul(self.base_grid.view(self.H * self.W, 3), M_inv_l.transpose(1, 2))
            grid_l = torch.div(grid_l[:, :, 0:2], grid_l[:, :, 2:])
            _grid_l = grid_l.clone()



            grid_l[:,:,0] = grid_l[:,:,0] / 3.
            grid_m[:,:,0] = (grid_m[:,:,0] + 1) / 3.
            grid_r[:,:,0] = (grid_r[:,:,0] + 2) / 3.


            grid_l = grid_l.detach().cpu().numpy()
            grid_m = grid_m.detach().cpu().numpy()
            grid_r = grid_r.detach().cpu().numpy()
            ...
            x_vals = _grid_r[0,:,0].detach().cpu().numpy() * width
            y_vals = _grid_r[0,:,1].detach().cpu().numpy() * height
            indicate_x1 = x_vals < width
            indicate_x2 = x_vals > 0
            indicate_y1 = y_vals < height
            indicate_y2 = y_vals > 0
            indicate = (indicate_x1 * indicate_x2 * indicate_y1 * indicate_y2)*1

            index_r = np.where(indicate==1)[0]
            # _index_r = np.tile(index_r, (bs,1))



            x_vals = _grid_m[0,:,0].detach().cpu().numpy() * width
            y_vals = _grid_m[0,:,1].detach().cpu().numpy() * height
            indicate_x1 = x_vals < width
            indicate_x2 = x_vals > 0
            indicate_y1 = y_vals < height
            indicate_y2 = y_vals > 0
            ...
            index_m = np.where(indicate==1)[0]
            # _index_m = np.tile(index_m, (bs,1))



            x_vals = _grid_l[0,:,0].detach().cpu().numpy() * width
            y_vals = _grid_l[0,:,1].detach().cpu().numpy() * height
            indicate_x1 = x_vals < width
            indicate_x2 = x_vals > 0
            indicate_y1 = y_vals < height
            indicate_y2 = y_vals > 0
            ...
            index_l = np.where(indicate==1)[0]
            # _index_l = np.tile(index_l, (bs,1))


            grid[:,index_r,:] = grid_r[:,index_r,:]
            grid[:,index_m,:] = grid_m[:,index_m,:]
            grid[:,index_l,:] = grid_l[:,index_l,:]


            # grid = Variable(torch.from_numpy(grid)).type(torch.float32).to(device)
            grid = torch.from_numpy(grid).type(torch.float32).to(device)
...
class SingleUpTopViewPathway(nn.Module):
    def __init__(self, input_channels, batch_norm=False, init_weights=True):
        super(SingleUpTopViewPathway, self).__init__()
